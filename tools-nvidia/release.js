...
,   fs = require("fs")
,   pth = require("path")
,   bwc = require("./build-nvidia-common")
...
// 1. Make sure you are up to date and on the nvidia-develop branch (git up; git checkout nvidia-develop)
function upToDateAndDev (cb) {
    prompt.get(
        {
            description:    "Are you up to date and on branch nvidia-develop"
        ,   pattern:        /^[yn]$/i
    ...
    ,   function (err, res) {
            ...
            if (val === "n") return cb("Make sure to run git up; git checkout nvidia-develop");
            cb();
        }
    );
}
...
function bumpVersion (cb) {
    ...
    ,   version = pack.match(/"versionNVIDIA"\s*:\s*"([\d\.]+)"/)[1]
    ;
    if (!version) cb("Version NVIDIA string not found in package.json");
    ...
    ,   function (err, res) {
            ...
            if (err) return cb(err);
            pack = pack.replace(/("versionNVIDIA"\s*:\s*")[\d\.]+(")/, "$1" + targetVersion + "$2");
...
// 3. Run the build script (node tools/build-nvidia-common.js). This should respond "OK!" (if not, fix the
//    issue).
// 4. Add the new build (git add builds/respec-nvidia-common-3.x.y.js).
// 5. Commit your changes (git commit -am v3.x.y)
// 6. Merge to gh-pages (git checkout gh-pages; git merge nvidia-develop)
...
function build (cb) {
    bwc.buildNVIDIA(targetVersion, cb);
}

function add (cb) {
    var path = rel("../builds/respec-nvidia-common-" + targetVersion + ".js");
...
function merge (cb) {
    exec("git merge pcisg-develop", cb);
}

function checkoutDevelop (cb) {
    exec("git checkout nvidia-develop", cb);
}

function tag (cb) {
    exec("git tag -s p" + targetVersion, cb);
}
