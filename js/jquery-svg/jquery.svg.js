...
$.extend(SVGWrapper.prototype, {
	...
	marker: function(parent, id, refX, refY, mWidth, mHeight, orient, settings) {
		let args = this._args(arguments, ['id', 'refX', 'refY', 'mWidth', 'mHeight', 'orient'], ['orient']);
	...
	style: function(parent, styles, settings) {
		...
		let node = this._makeNode(args.parent, 'style', $.extend({type: 'text/css'}, args.settings || {}));
	...
	script: function(parent, script, type, settings) {
		...
		let node = this._makeNode(args.parent, 'script', $.extend(
	...
	linearGradient: function(parent, id, stops, x1, y1, x2, y2, settings) {
		let args = this._args(arguments, ['id', 'stops', 'x1', 'y1', 'x2', 'y2'], ['x1']);
		let sets = $.extend({id: args.id},
	...
	radialGradient: function(parent, id, stops, cx, cy, r, fx, fy, settings) {
		let args = this._args(arguments, ['id', 'stops', 'cx', 'cy', 'r', 'fx', 'fy'], ['cx']);
		let sets = $.extend({id: args.id},
	...
	pattern: function(parent, id, x, y, width, height, vx, vy, vwidth, vheight, settings) {
		let args = this._args(arguments, ['id', 'x', 'y', 'width', 'height', 'vx', 'vy', 'vwidth', 'vheight'], ['vx']);
		let sets = $.extend({id: args.id, x: args.x, y: args.y, width: args.width, height: args.height},
	...
	svg: function(parent, x, y, width, height, vx, vy, vwidth, vheight, settings) {
		let args = this._args(arguments, ['x', 'y', 'width', 'height', 'vx', 'vy', 'vwidth', 'vheight'], ['vx']);
		let sets = $.extend({x: args.x, y: args.y, width: args.width, height: args.height},
			(args.vx !== null ? {viewBox: args.vx + ' ' + args.vy + ' ' + args.vwidth + ' ' + args.vheight} : {}));
	...
	use: function(parent, x, y, width, height, ref, settings) {
		...
		let node = this._makeNode(args.parent, 'use', $.extend(
	...
	image: function(parent, x, y, width, height, ref, settings) {
		let args = this._args(arguments, ['x', 'y', 'width', 'height', 'ref']);
		let node = this._makeNode(args.parent, 'image', $.extend(
			{x: args.x, y: args.y, width: args.width, height: args.height}, args.settings || {}));
		node.setAttributeNS($.svg.xlinkNS, 'href', args.ref);
		return node;
	},
	...
	rect: function(parent, x, y, width, height, rx, ry, settings) {
		let args = this._args(arguments, ['x', 'y', 'width', 'height', 'rx', 'ry'], ['rx']);
	...
	textpath: function(parent, path, value, settings) {
		...
		let node = this._text(args.parent, 'textPath', args.value, args.settings || {});
	...
	add: function(parent, node) {
		let args = this._args((arguments.length === 1 ? [null, parent] : arguments), ['node']);
		let svg = this;
	...
	load: function(url, settings) {
		...
		let size = [this._svg.getAttribute('width'), this._svg.getAttribute('height')];
		let wrapper = this;
		...
		let loadSVG = function(data) {
			...
			if (data.documentElement.nodeName !== 'svg') {
				...
				let messages = (errors.length ? errors[0].getElementsByTagName('div') : []); // Safari
