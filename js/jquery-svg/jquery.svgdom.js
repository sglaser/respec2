		...
		return this.each(function() {
			if ($.svg.isSVGElem(this)) {
				...
				$.each(classNames.split(/\s+/), function(i, className) {
					let classes = getClassNames(node);
...
$.fn.toggleClass = function(origToggleClass) {
	return function(classNames, state) {
		...
		let origArgs = arguments;
		let hasState = (typeof state === 'boolean');
		return this.each(function() {
			if ($.svg.isSVGElem(this)) {
				if (typeof classNames === 'string') {
					...
					let classes = getClassNames(this);
					if (classes) {
...
$.fn.attr = function(origAttr) {
	return function(name, value, type) {
		if (typeof name === 'string' && value === undefined) { // Return attribute value
			let val = origAttr.apply(this, arguments);
			if (val && val.baseVal && val.baseVal.numberOfItems != null) { // Multiple values
				value = '';
				val = val.baseVal;
				if (name === 'transform') {
					for (let i = 0; i < val.numberOfItems; i++) {
						let item = val.getItem(i);
						switch (item.type) {
...
if ($.cssProps) {
	$.css = function(origCSS) {
		return function(elem, name, numeric, extra) {
			let value = (name.match(/^svg.*/) ? $(elem).attr($.cssProps[name] || name) : '');
