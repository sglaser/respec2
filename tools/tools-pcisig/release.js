...
,   fs = require("fs")
,   pth = require("path")
,   bwc = require("./build-pcisig-common")
...
// 1. Make sure you are up to date and on the pcisig-develop branch (git up; git checkout pcisig-develop)
function upToDateAndDev (cb) {
    prompt.get(
        {
            description:    "Are you up to date and on branch pcisig-develop"
        ,   pattern:        /^[yn]$/i
    ...
    ,   function (err, res) {
            ...
            if (val === "n") return cb("Make sure to run git up; git checkout pcisig-develop");
            cb();
        }
    );
}
...
function bumpVersion (cb) {
    ...
    ,   version = pack.match(/"versionPCISIG"\s*:\s*"([\d\.]+)"/)[1]
    ;
    if (!version) cb("Version PCISIG string not found in package.json");
    ...
    ,   function (err, res) {
            ...
            if (err) return cb(err);
            pack = pack.replace(/("versionPCISIG"\s*:\s*")[\d\.]+(")/, "$1" + targetVersion + "$2");
...
// 3. Run the build script (node tools/build-pcisig-common.js). This should respond "OK!" (if not, fix the
//    issue).
// 4. Add the new build (git add builds/respec-pcisig-common-3.x.y.js).
// 5. Commit your changes (git commit -am v3.x.y)
// 6. Merge to gh-pages (git checkout gh-pages; git merge pcisig-develop)
...
function build (cb) {
    bwc.buildPCISIG(targetVersion, cb);
}

function add (cb) {
    var path = rel("../builds/respec-pcisig-common-" + targetVersion + ".js");
...
function checkoutDevelop (cb) {
    exec("git checkout pcisig-develop", cb);
}
